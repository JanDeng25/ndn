...
//进入一个新的路段后的fib表格变化？
void NrFibImpl::laneChange(std::string oldLane, std::string newLane)
{
	if (oldLane.empty()
	...
	{//旧路段在fib头部才进行删除
			...
			//报错？
		//NS_ASSERT_MSG(IsOldLaneAtFibBegin,"The old lane should at the beginning of the fibContainer. Please Check~");
		//1. Befor erase it, cancel all the counting Timer fore the neighbor to expire
		DynamicCast<EntryNrImpl>(*it)->RemoveAllTimeoutEvent();
