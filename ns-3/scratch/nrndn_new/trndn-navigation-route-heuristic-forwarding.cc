...
void TrNavigationRouteHeuristic::Start()
{
	...
	Simulator::Schedule (Seconds (50), & TrNavigationRouteHeuristic::DeleteTable, this);
}
...
void TrNavigationRouteHeuristic::DeleteTable()
{
	m_pit->cleanPIT();
	m_fib->cleanFIB();
	Simulator::Schedule (Seconds (10.0), & TrNavigationRouteHeuristic::DeleteTable, this);
}
		...
		Ptr<Interest> interest)
{
	...
	double interval = fabs(400 - distance) ;
	//cout<<"into on interest1"<<endl;
	if(INTEREST_PACKET == interest->GetScope())
	{
		if(!isDuplicatedInterest(nodeId,seq) )
		{
			...
			{
				//cout<<"into on interest2"<<endl;
				Time sendInterval = MilliSeconds(m_uniformRandomVariable->GetInteger(0, 100));
...
void TrNavigationRouteHeuristic::OnData(Ptr<Face> face, Ptr<Data> data)
{
	...
	double disY =m_sensor->getY() - y;
	double distance =sqrt( disX * disX + disY * disY);
	double interval = fabs(500 - distance) ;

	if(RESOURCE_PACKET == packetTypeTag.Get())
	{
		//cout<<"into on data1"<<endl;
		if(!isDuplicatedData(nodeId,signature))
		{
			...
			if( !m_fib->Find(data->GetName()) && distance<300)
			{
...
void TrNavigationRouteHeuristic::ForwardInterestPacket(Ptr<Interest> src)
{
	...
	{
		Simulator::Schedule (Seconds (3), & TrNavigationRouteHeuristic::ForwardInterestPacket, this,src);
		return;
	}
...
void TrNavigationRouteHeuristic::ReplyDataPacket(Ptr<Interest> interest)
{
	...
	uint32_t nextid = nrheader.getCurrentId();
	nrheader.setX(m_sensor->getX());
	nrheader.setY(m_sensor->getY());
	nrheader.setCurrentId(m_node->GetId());
	...
	cout<<"node: "<<m_node->GetId()<<" reply data packet to "<<nrheader.getSourceId()<<" next id:"<<nextid<<endl;
...
void TrNavigationRouteHeuristic::PrepareInterestPacket(Ptr<Interest> interest)
{
	...
	ndn::nrndn::trndnHeader nrheader;
	nrPayload->RemoveHeader(nrheader);
	//cout<<"into prepare interest packet1"<<endl;
	...
	//cout<<"into prepare interest packet2"<<endl;
	...
	FwHopCountTag hopCountTag;
	nrPayload->AddPacketTag(hopCountTag);
	//cout<<"into prepare interest packet3"<<endl;
	ndn::nrndn::PacketTypeTag typeTag(INTEREST_PACKET);
	nrPayload->AddPacketTag (typeTag);
	//cout<<"into prepare interest packet4"<<endl;
	interest->SetPayload(nrPayload);
	Simulator::Schedule(
					MilliSeconds(m_uniformRandomVariable->GetInteger(0, 100)),
	...
	//SendInterestPacket(interest);
	cout<<"node: "<<m_node->GetId()<<"  send interest packet,name: "<<interest->GetName().toUri()<<" scope:"<<(int)(interest->GetScope())<<" next hop: "<<nextid<<endl;
//getchar();
	m_interestNonceSeen.Put(interest->GetNonce(),true);
