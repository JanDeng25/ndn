...
{
	uint32_t random = m_rand.GetInteger(1, 100);
	if(random > 30)
		return;

	uint32_t delay = m_rand.GetInteger(70, 300);
...
void nrConsumer::SendPacket()
{
	...
	//cout<<"consumer send packet"<<endl;
	  ...
	  uint32_t num = m_rand.GetInteger(1, 99);
	  Name prefix("/");
...
void nrConsumer::OnData(Ptr<const Data> data)
{
	...
	cout<<m_node->GetId() <<" consumer on data"<<endl;
