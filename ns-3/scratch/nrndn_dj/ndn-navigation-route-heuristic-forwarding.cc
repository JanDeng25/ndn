...
void NavigationRouteHeuristic::Start()
{
	...
	if(m_node->GetId() <10)
	{
		...
		{
			uint32_t num = m_node->GetId() * 10 + i;
			if(num > 99)
				num -= (m_node->GetId() * 10 + 10);
		...
		Ptr<Interest> interest)
{
	...
	//800为标准时，1277仍为负
	double interval = (1000 - distance) *1.5;
...
void NavigationRouteHeuristic::OnData(Ptr<Face> face, Ptr<Data> data)
{
	...
	if(TABLE_PACKET == packetTypeTag.Get())//收到其他节点回复的table packet（非请求table的表格）
	{
		...
		if(m_sensor->getLane() == tableheader.getCurrentLane())
		{
	...
	{
		//cout << "node: " << m_node->GetId() << " into RESOURCE_PACKET / onData / forwarder" << endl;
...
void NavigationRouteHeuristic::ReplyTablePacket(Ptr<Interest> interest)
{
	...
	vector<string> sourcename = nrheader.getLaneList();
	...
	for(uint32_t i = 0; i < sourcename.size(); ++i)
	{
/*
		Name name(sourcename[i]);
		if(m_pit->Find(name))
		{
			std::vector<Ptr<pit::Entry> >::iterator pit = m_pit->Find(name);
			Ptr<pit::nrndn::EntryNrImpl> tempPitEntry = DynamicCast<ndn::pit::nrndn::EntryNrImpl>(*pit);
			tableheader.addPIT(tempPitEntry);
		}
		if(m_fib->Find(name))
		{
			std::vector<Ptr<fib::Entry> >::iterator fib = m_fib->Find(name);
			Ptr<fib::nrndn::EntryNrImpl> tempFibEntry = DynamicCast<ndn::fib::nrndn::EntryNrImpl>(*fib);
			tableheader.addFIB(tempFibEntry);
		}
*/
	}
...
void NavigationRouteHeuristic::laneChange(std::string oldLane, std::string newLane)
{
	...
	for(uint32_t i = 0; i < m_pit->getPIT().size(); ++i)
	{
		Ptr<pit::nrndn::EntryNrImpl> tempPitEntry = DynamicCast<ndn::pit::nrndn::EntryNrImpl>(m_pit->getPIT()[i]);
		std::unordered_set< std::string > nexthop =tempPitEntry->getIncomingnbs();
		std::unordered_set< std::string >::iterator it;
		for(it =nexthop.begin(); it != nexthop.end(); ++it )
		{
			if(!IsConnected((*it), m_sensor->getLane()) && !isSameLane((*it), m_sensor->getLane()))
			{
				//删除该下一跳，替换为m_sensor->getLane()
			}
			else if(isSameLane((*it), m_sensor->getLane()))
			{
				sourcename.push_back(tempPitEntry->getEntryName());
				break;
			}
		}
	}
	...
	for(uint32_t i = 0; i < m_fib->getFIB().size(); ++i)
	{
		...
		{
			//下一跳替换为m_sensor->getLane()，TTL+1
		}
		else if (isSameLane(hop, m_sensor->getLane()))
		{
			sourcename.push_back(tempFibEntry->getEntryName());
		}
	}
}

void NavigationRouteHeuristic::CheckTable()
{
	vector<string> sourcename;
	for(uint32_t i = 0; i <100; ++ i)
	{
		Name name("/");
		name.appendNumber(i);
		if(!m_pit->Find(name) || !m_fib->Find(name))
...
bool NavigationRouteHeuristic::IsConnected(string lane1, string lane2)//判断两路段是否相连
{
